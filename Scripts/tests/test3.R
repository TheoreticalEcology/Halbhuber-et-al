...
#### Hyperparameter Grid ####
hyper_grid <- function(lambda, lr, dropout) {
  expand.grid(
    lr = lr,
    dropout = dropout,
    lambda = lambda,
...
hyper_grid_object <- hyper_grid(lr = c(0.01), dropout = c(0.1), lambda = c(0.1))
...
for (desc in descriptors) {
  ...
  for (fold_column in folds) {
    for (trait in trait_levels) {
      ...
      best_lambda <- hp_list$lambda[best_index]
