...
df <- readRDS("/home/isabellehalbhuber/paper/EcoToxTM/Data/df_final_tokenizedSMILES.rds")
df <- na.omit(df)
df <- df %>% select(-SMILES)


## Embedding von 40 auf 80

BiRNN <- nn_module(
  initialize = function(embedding = 40L, rnn_hidden_size = 20L, rnn_layers = 3L, max_len = 177L, num_embeddings = 615) {
...
r2 <- as.data.frame(lapply(results, get_r2))

saveRDS(r2, file = "Results/Attention_traits.rds")
