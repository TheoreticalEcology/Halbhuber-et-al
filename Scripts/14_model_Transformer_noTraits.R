...
transformer = nn_module(
  ...
  forward = function(tokens, traits) {
    ...
    if (!is.null(traits)) {
      mask <- torch_cat(list(mask, torch_zeros(tokens$shape[1], traits$shape[2], 
                                               dtype=torch_bool(), device=tokens$device)), dim = 2)
    }
...
hyper_grid_object <- hyper_grid(lr = c(0.001, 0.0001), weight_decay = c(0.000001))
...
for (fold_column in fold_columns) {
  ...
  for (j in 1:nhyper) {
    ...
    for (i in 1:(k - 2)) {
      ...
      model <- transformer(traits = NULL, num_layers = 5L, num_heads = 3L, emb_dim = 36L)
      ...
      for (epoch in 1:nepochs) {
        ...
        for (l in seq_along(dataloader_train)) 
          ...
          print(dim(batch_tokens))
          output <- model(batch_tokens, traits = NULL)
          print(dim(output))
  ...
  # train best model
  model_val <- transformer(traits = NULL, num_layers = 5L, num_heads = 3L, emb_dim = 36L)
  ...
  for (epoch in 1:nepochs) {
    ...
    for (n in seq_along(dataloader_val_train)) {
      ...
      output <- model_val(batch_tokens, traits = NULL)
      loss <- loss_fn(output, batch_LC50$unsqueeze(2L))
      loss$backward()
      optimizer_val$step()
...
#saveRDS(average_r2_summary, file = "~/paper/EcoToxTM/Results/Transformer_noTraits_r2_summary.rds")
...
saveRDS(hyperparameters$Transformer,file = "~/paper/EcoToxTM/Results/Transformer_noTraits_SMILES.rds")
