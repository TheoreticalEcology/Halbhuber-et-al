  ...
  for (fold_column in folds) {
    for (trait in trait_levels) {
      ...
      for (j in 1:nhyper) {
        ...
        val_train  <- df[!df[[fold_column]] %in% c(k - 1, k), ]
        ...
        # Save in corresponding lists for three dimensions (folds)
        ...
        hyperparameters$randomForest[[trait]][[desc]][[fold_column]]$val[j]   <- R_squared_val
        
        cat("Saved for", trait, " | ", desc, " | Fold: ", fold_column, "\n")
      }
    }
  }
}
